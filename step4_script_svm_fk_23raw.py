...
Parameters:
    – cross_val:True/False (default) choice between using C+/C-=20
...
if cross_val==True:
    ...
    print('Cross-validation skipped ... Using C+/C-=20')
    opt_params_svm_fk={'class_weight':{0: 0.05, 1: 1}}
...
for yr in range_oos:
    ...
    ndcg_svm1[m]=ndcg_k(Y_OOS.values,probs_oos_fraud_svm,99)
    ...
    ndcg_svm5[m]=ndcg_k(Y_OOS.values,probs_oos_fraud_svm,95)
    ...
    ndcg_svm10[m]=ndcg_k(Y_OOS.values,probs_oos_fraud_svm,90)
...
perf_tbl_general['Roc']=np.mean(roc_svm[2:7])
...
perf_tbl_general['Sensitivity']=np.mean(sensitivity_OOS_svm[2:7])
    
perf_tbl_general['Specificity']=np.mean(specificity_svm[2:7])

perf_tbl_general['Precision']=np.mean(precision_svm[2:7])
