...
1) In a terminal run the following commands to create the conda development environment:
...
The environment file already installs this package in editable mode via the `pip` section, so no separate `conda develop` step is needed.

1) In a Python environment run the following to load the module:

  `from MLFraud_module import ML_Fraud as mf`

1) Define your settings as:
...
1) Choose any of the following methods:
